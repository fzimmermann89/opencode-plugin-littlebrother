  ...
  const performSupervisorCheck = async (sessionID: string, state: SessionState) => {
    ...
    try {
      ...
      if (decision.status === 'ABORT') {
        ...
        await injectSupervisorMessage(client, sessionID, `Session aborted: ${decision.reason}`, 'error')
      ...
      if (!config.failOpen) {
        ...
        try {
          ...
          logger.error('Failed to abort session after supervisor failure', { sessionID, error: String(abortErr) })
        }
        return
      }
