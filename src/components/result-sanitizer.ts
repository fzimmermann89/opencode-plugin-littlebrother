...
export function createResultSanitizer(
  ...
  const handler = async (input: ToolExecuteInput, output: ToolExecuteAfterOutput): Promise<void> => {
    ...
    if (config.sanitizer.deepAnalysis && !modified && content.length > 1000) {
      try {
        ...
        if (decision.status === 'REDACT' && decision.replacement) {
          ...
          modified = true
          logger.info('Deep analysis triggered redaction', { tool: input.tool, reason: decision.reason })
        ...
        await showToast(client, 'Supervisor unavailable - sanitizer deep analysis skipped', 'warning')
      }
    }
