...
export class GatekeeperBlockError extends Error {
  constructor(reason: string) {
    ...
    // Required for proper instanceof checks when extending Error in transpiled code
    Object.setPrototypeOf(this, GatekeeperBlockError.prototype);
  }
}
...
export function createActionGatekeeper(
  ...
  const handler = async (
    ...
    try {
      ...
      await showToast(client, `🛡️ Gatekeeper: checking ${input.tool}...`, 'info');

      const decision = await supervisorClient.query(input.sessionID, 'gatekeeper', payload, {
        userGoal,
      });
      ...
      if (config.failOpen) {
        await showToast(
          client,
          `Supervisor unavailable - allowing ${input.tool} (fail-open)`,
          'warning'
        );
        return;
      }
      ...
      await showToast(
        client,
        `Supervisor unavailable - blocking ${input.tool} (fail-closed)`,
        'error'
      );
